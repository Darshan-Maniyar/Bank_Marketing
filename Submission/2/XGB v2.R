...
set.seed(2)
bst <- xgboost(param=param,xgbd ,nrounds=222 ,nfold = 4,seed=27,verbose = 0)#,niter=299, early_stopping_rounds=50,min_child_weight=10,subsample =0.9)#
...
set.seed(2)
btr$pred=predict(bst,xgbd,ntreelimit = 222,type='')
hist(btr$pred)
...
bt=b_test
set.seed(2)
bt$pred=predict(bst,xgbdtest,ntreelimit = 222)#as.matrix(h1_test[2:425])
hist(bt$pred)

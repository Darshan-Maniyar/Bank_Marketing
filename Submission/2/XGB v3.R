...
bank$pdays[bank$pdays==-1]=9999999999
bank$pdays_1=ifelse(bank$pdays==9999999999,0,1)
...
b=dummy(b,c("marital","job","education","default","housing","loan","contact","month"),limit = 50,rm_original = T)
...
set.seed(2)
bst.cv = xgb.cv(param=param,  xgbd ,nrounds=500 ,early_stopping_rounds=50,nfold = 4,seed=27,"maximize"=T)#,niter=10,  ,niter=500, nrounds = 50,colsample_bytree =0.9,gamma =100,min_child_weight=10,subsample =0.9)#)#prediction=TRUE, verbose=FALSE,
bst.cv$niter
bst.cv$best_iteration
max.auc.idx = which.max(bst.cv$evaluation_log[,test_KS_mean]) 
max.auc.idx 
...
set.seed(2)
bst <- xgboost(param=param,xgbd ,nrounds=103 ,nfold = 4,seed=27,verbose = 0)#,niter=299, early_stopping_rounds=50,min_child_weight=10,subsample =0.9)#
...
set.seed(2)
btr$pred=predict(bst,xgbd,ntreelimit = 103,type='')
hist(btr$pred)
a2=confmatrix(btr$y,btr$pred,model = "KS")
View(a2$table)

a2=table(btr$y,as.numeric(btr$pred>0.5))
...
bt=b_test
set.seed(2)
bt$pred=predict(bst,xgbdtest,ntreelimit = 103)#as.matrix(h1_test[2:425])
hist(bt$pred)
...
a2=table(bt$y,as.numeric(bt$pred>0.5))
(a2[2,2]/sum(a2[2,]))- (a2[1,2]/sum(a2[1,]))
(a2[2,2]/(a2[2,2]+a2[1,2]+a2[2,1]))


sp <- function(preds, y) {
  label=getinfo(y,"label")
  k=(table(label,preds>0.5))
  if((nrow(k)+ncol(k))==4){
    k1=(k[2,2]/(k[2,2]+k[1,2]+k[2,1]))}
  else{k1=0}
...
bank$pred=predict(bst,xgbd,ntreelimit = 222)#as.matrix(h1_test[2:425])
hist(bank$pred)

bank$y=as.numeric(bank$pred>0.5)
table(bank$y)
3244/10320
write.csv(bank$y, file = "4_xgb.csv",row.names = F)

...
# bank$age_code2 = 0
# bank$age_code3 = 0
bank = bank %>%  mutate ( age_code1 = ifelse(bank$age<23,1,0),
                          age_code2 = ifelse(bank$age>=23 & bank$age < 61,1,0),
                          age_code3 = ifelse(bank$age>=61 ,1,0)
                          ) %>% select(-age,age_code1)
...
set.seed(1)
fitrf=randomForest(y~.,mtry=10 ,
                   sampsize=c(3000,2800),data=bank_train,ntree=100)
fitrf
varImpPlot(fitrf)
plot(fitrf)

set.seed(1)
fitrf=randomForest(y~.-age_code1-default-age_code3-job_code3-marital_divorced-loan-education_secondary ,mtry=10 ,
                   sampsize=c(2000,2800),data=bank_train,ntree=100)
fitrf
varImpPlot(fitrf)
plot(fitrf)
